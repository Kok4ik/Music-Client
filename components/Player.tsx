...
const Player = () => {
    ...
    let albumTracks: any = activeAlbum?.tracks
    const nextTrack = () => {
        if (activeAlbum && activeAlbum.tracks.length > albumTracks.indexOf(active?._id) + 1) {
            axios.get(process.env.NEXT_PUBLIC_SECRET_HOST + '/tracks/' + albumTracks[albumTracks.indexOf(active?._id) + 1])
    ...
    const lastTrack = () => {
        if (activeAlbum && albumTracks.indexOf(active?._id) > 0) {
            axios.get(process.env.NEXT_PUBLIC_SECRET_HOST + '/tracks/' + albumTracks[albumTracks.indexOf(active?._id) - 1])
                .then(response => setActiveTrack(response.data))
            playTrack()
            audio.play()
